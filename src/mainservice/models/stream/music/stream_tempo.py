...
def predict(data,fs=22050):
	...
	# Select a part of it
	rawcorr = xcorr[int((len(xcorr)-1)/2)+np.arange(acmax+1)-1]
	...
	result_dict = {}
	result_dict['tempo'] = np.array(( round(tempos[0],4), round(tempos[1],4) )).tolist()
	result_dict['prob'] = np.array(( round(p_ratio1,4),round(p_ratio2,4) )).tolist()
