...
def predict(data,fs=22050, profile_type='Krumhansl', distance_type = 'KL', n_key=1):
	...
	if distance_type == 'Corr':
		...
		cor = np.zeros(24)
		for j in range(24):
			cor[j] = scipy.stats.entropy(pk=ch_avg+np.finfo(float).eps,qk=key_prof[j,:])
		# key_idx = np.argmin(cor)
		key_idx = cor.argsort()[:n_key][0::]

		# Probability
		cor = (cor-np.sum(cor))/np.std(cor)*1*(-1)
	...
	result_dict['prob'] = np.around(key_prob,4)[0]
